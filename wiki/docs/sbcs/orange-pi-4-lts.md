---
title: Orange Pi 4 Lts
description: Orange Pi 4 Lts — single-board computer powered by Rockchip RK3399.
generated: 2026-03-30 12:33:56
---

# Orange Pi 4 Lts

Orange Pi 4 Lts — single-board computer powered by Rockchip RK3399.


## Hardware & Compatibility

<div data-reg-compat="orange-pi-4-lts:specs"></div>
<script src="https://compat.reglinux.org/widget.js"></script>

## Installation notes

1. Grab the image. Download the latest Orange Pi 4 Lts build from the downloads page.

2. Flash to storage. Use balenaEtcher, Raspberry Pi Imager, or `dd` to write the image to an SD card or eMMC. Make sure to verify the checksum before booting.

3. First boot setup. Insert the media into your Orange Pi 4 Lts, power on, and let REG Linux expand the filesystem. Pair controllers inside EmulationStation once prompted.

## Resources

- REG Linux download page: https://reglinux.org/download/orange-pi-4-lts/
