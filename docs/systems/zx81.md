...
RetroArch exposes the usual `zx81.*` options (`videomode`, `shaders`, `pixel_perfect`, etc.). Use the Quick Menu (`[HOTKEY]` + ![south](/wiki/south.png)) to tweak the display and the input device type. X-Keys also toggle features like `global.81_chroma_81` for colorization and `global.81_highres` for the WRX high-resolution mode.
...
- For broader RetroArch tips, visit the [generic support pages](/support).
